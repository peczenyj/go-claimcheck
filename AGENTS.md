...
- Conditional offload: `WrapTopic` publishes messages smaller than `Options.MinSize` inline (no blob); `MinSize=0` (default) always offloads. Send-side only; the core `Offload` always writes a blob.
