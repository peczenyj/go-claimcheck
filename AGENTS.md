...
| `task mock` | Generates mocks using `mockery` (configured in `.mockery.yaml`). |
...
- For components requiring external dependencies (like cloud providers), use the Go CDK `memdriver` or `memblob` for in-memory testing when possible, or mocks generated by `mockery`.
